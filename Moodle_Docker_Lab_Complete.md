...
คำตอบ:
```
Docker Image คือ ไฟล์ต้นแบบ (Template) ที่เก็บโครงสร้างระบบ เช่น OS, โปรแกรม, ไลบรารี และการตั้งค่า
Docker Container คือ อินสแตนซ์ที่รันอยู่จริงซึ่งสร้างมาจาก Image
```
...
```
moodle_app
ทำหน้าที่รันเว็บแอป Moodle (PHP + Apache)

db (MariaDB/MySQL)
ทำหน้าที่เก็บข้อมูลทั้งหมด เช่น ผู้ใช้ รายวิชา คะแนน
```
...
คำตอบ:
```
Network ใน Docker Compose ช่วยให้ Container ต่าง ๆ สามารถสื่อสารกันได้โดยใช้ชื่อ Service เป็น hostname เช่น moodle_app สามารถเชื่อมต่อ db ได้โดยใช้ hostname "db"
```
...
คำตอบ:
```
`depends_on` ใช้กำหนดลำดับการเริ่มต้น Container โดย Container ที่ระบุใน `depends_on` จะถูกเริ่มต้นก่อน Container ที่ใช้ `depends_on`
```
...
คำตอบ:
```
ในส่วนของ service moodle_app ให้แก้ไข ports จาก "80:80" เป็น "9000:80"
```
...
คำตอบ:
```
หมายถึง Moodle จะเชื่อมต่อฐานข้อมูลผ่าน container ชื่อ db

ไม่ใช้ localhost เพราะ:
ใน Docker แต่ละ container มี localhost ของตัวเอง
ถ้าใช้ localhost จะชี้ไปที่ container ตัวเอง ไม่ใช่ database
```
...
- แยกแอปและฐานข้อมูลอย่างชัดเจน
- ง่ายต่อการจัดการและอัพเดต
ข้อเสียของ Docker:
- ต้องเรียนรู้การใช้งาน Docker
- อาจมีปัญหาเรื่อง Performance ในบางกรณี

ติดตั้งปกติ (Manual Install)
ข้อดี:
- ควบคุมได้ละเอียดมากขึ้น
ข้อเสีย:
- ติดตั้งยุ่งยากและใช้เวลานาน
```
...
```
วิธีตรวจสอบ:
1. ตรวจสอบสถานะ Container db
   docker-compose ps
2. ดู Logs ของ moodle_app
...
วิธีแก้ไข:
- ตรวจสอบชื่อ service ว่าตรงกับ MOODLE_DB_HOST
- ตรวจสอบรหัสผ่าน DB
...
คำตอบ:
```
`docker-compose down -v` จะหยุด Container ทั้งหมด และลบ Volume ที่เกี่ยวข้องด้วย ซึ่งหมายความว่าข้อมูลทั้งหมดใน Volume (เช่น moodledata) จะถูกลบไปด้วย
